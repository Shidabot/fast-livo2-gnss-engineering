...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  if (!std::isfinite(IMU_mean_acc_norm) || IMU_mean_acc_norm < 1e-6) {
    ROS_ERROR("Invalid mean acceleration during IMU initialization");
    Reset();
    return;
  }
  ...
  state_inout.bias_g = ba_bg_est_en ? mean_gyr : Zero3d;
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      const double segment_begin = std::max(prop_beg_time, head->header.stamp.toSec());
      // The final pair also supplies the existing short tail extrapolation.
      const double segment_end = i == v_imu.size() - 2 ? prop_end_time :
                                 std::min(prop_end_time, tail->header.stamp.toSec());
      if (segment_end <= segment_begin) continue;
      dt = segment_end - segment_begin;
      offs_t = segment_end - prop_beg_time;
...
bool ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_, Sophus::SE3 T_G_to_W)
{
  double t1, t2, t3;
  t1 = omp_get_wtime();
  cur_pcl_un_->clear();
  if (!lidar_meas.lidar || lidar_meas.measures.empty()) return false;
  if (!imu_en)
  {
    if (lidar_meas.lidar->empty()) return false;
    Forward_without_imu(lidar_meas, stat, *cur_pcl_un_);
    return true;
  }

  MeasureGroup meas = lidar_meas.measures.back();
  const double target_time = lidar_meas.lio_vio_flg == LIO ? meas.lio_time : meas.vio_time;
  if (!std::isfinite(target_time) || target_time < 0.0) return false;
  double previous_time = -1.0;
  for (const auto &imu : meas.imu) {
    if (!imu) return false;
    const double time = imu->header.stamp.toSec();
    ...
    if (!std::isfinite(time) || time < previous_time ||
      ...
      ROS_ERROR("Invalid IMU batch: non-finite values or decreasing timestamps");
      return false;
    }
    previous_time = time;
  }
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT && IMU_mean_acc_norm > 1e-6)
    {
      // cov_acc *= pow(G_m_s2 / mean_acc.norm(), 2);
      imu_need_init = false;
      // The initialized stationary state is anchored at the last consumed IMU.
      last_prop_end_time = last_imu->header.stamp.toSec();
      angvel_last = mean_gyr - stat.bias_g;
      acc_s_last = stat.rot_end * (mean_acc * G_m_s2 / IMU_mean_acc_norm - stat.bias_a) + stat.gravity;
  ...
  if (!runtime_checks::validInterval(last_prop_end_time, target_time)) {
    ROS_ERROR("Rejecting backward IMU propagation; restart the session after a clock reset");
    return false;
  }
  // VIO immediately after LIO observes the same state time and consumes no IMU.
  if (meas.imu.empty() && target_time > last_prop_end_time + 1e-9) return false;
