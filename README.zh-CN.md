...
**维护者：** shida · [shida.86@outlook.com](mailto:shida.86@outlook.com)

这是一个正在进行工程调试的个人项目，重点是传感器接入、时间与坐标一致性、初始化、运行可靠性、配置校验和可重复测试。

本项目不宣称学术贡献或新的 SLAM 方法，也不是 FAST-LIVO2 官方发布版本。激光雷达–视觉–惯性前端源自 FAST-LIVO2，当前代码还继承了已有 FAST-LIVO2-RTK 扩展。上游算法和继承代码的贡献属于原作者。

> **当前状态：** 持续开发与调试中。完整 ROS 编译和真实传感器数据回放尚未完成验证，不能据此宣称具备生产部署条件或已经达到某种轨迹精度。
...
| Codex（OpenAI AI 编程助手） | 辅助代码审查、修复方案、回归测试工具、CI 配置和双语文档 |
...
原始激光雷达–视觉–惯性实现源自 **FAST-LIVO2**，由 Chunran Zheng 和上游贡献者开发。当前代码还继承了 FAST-LIVO2-RTK 扩展。shida 的作者署名覆盖本地修改与工程维护，上游算法和继承代码仍归属原作者。
