      ...
      extR(M3D::Identity())
{
  ...
  path.header.stamp = ros::Time().fromSec(state_time_);
...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  using runtime_checks::require;
  runtime_checks::finiteVector(extrinT, 3, "extrin_calib/extrinsic_T");
  runtime_checks::rotation(extrinR, "extrin_calib/extrinsic_R");
  if (img_en) {
    runtime_checks::finiteVector(cameraextrinT, 3, "extrin_calib/Pcl");
    runtime_checks::rotation(cameraextrinR, "extrin_calib/Rcl");
  } else {
    cameraextrinT = {0, 0, 0};
    cameraextrinR = {1, 0, 0, 0, 1, 0, 0, 0, 1};
  }
  require(lidar_en == 1 && (img_en == 0 || img_en == 1), "Supported modes require LiDAR and img_en=0 or 1");
  require(!img_en || imu_en, "LIVO requires IMU enabled");
  require(!lid_topic.empty() && !imu_topic.empty() && (!img_en || !img_topic.empty()), "Sensor topics must not be empty");
  require(imu_int_frame > 0 && max_iterations > 0 && grid_size > 0 && grid_n_height > 0 &&
          patch_size >= 2 && patch_size % 2 == 0 && patch_pyrimid_level >= 1 && patch_pyrimid_level <= 12,
          "Invalid IMU initialization or visual iteration/grid/patch settings");
  require(p_pre->point_filter_num > 0 && p_pre->N_SCANS > 0 && p_pre->lidar_type >= AVIA && p_pre->lidar_type <= ROBOSENSE,
          "Invalid preprocessing settings");
  require(std::isfinite(p_pre->blind) && p_pre->blind >= 0.0, "preprocess/blind must be finite and nonnegative");
  require(pcd_save_interval == -1 || pcd_save_interval > 0, "pcd_save/interval must be -1 or positive");
  for (double offset : {exposure_time_init, img_time_offset, imu_time_offset, lidar_time_offset})
    require(std::isfinite(offset), "Time offsets must be finite");
  runtime_checks::positive(filter_size_surf_min, "preprocess/filter_size_surf");
  runtime_checks::positive(filter_size_pcd, "pcd_save/filter_size_pcd");
  runtime_checks::positive(gyr_cov, "imu/gyr_cov");
  runtime_checks::positive(acc_cov, "imu/acc_cov");
  runtime_checks::positive(IMG_POINT_COV, "vio/img_point_cov");
  runtime_checks::positive(inv_expo_cov, "vio/inv_expo_cov");
  p_pre->blind_sqr = p_pre->blind * p_pre->blind;

  // gps_en controls the batch backend. The experimental online fusion remains disabled.
  rtk_en = false;
  nh.param<vector<double>>("gps/extrinsic_T", T_I_R, vector<double>{0.0, 0.0, 0.0});
  nh.param<std::string>("laserMapping/outputfilepath", save_directory, "");
  runtime_checks::finiteVector(T_I_R, 3, "gps/extrinsic_T");
  require(!debug_mode || !save_directory.empty(), "Recording requires laserMapping/outputfilepath");
  if (!save_directory.empty()) std::filesystem::create_directories(save_directory + "/debug/pcd");
  ROS_INFO_STREAM("Effective mode: " << (img_en ? "LIVO" : imu_en ? "LIO" : "LO")
                  << "; IMU initialization samples: " << imu_int_frame
                  << "; offsets lidar/imu/image: " << lidar_time_offset << "/" << imu_time_offset << "/" << img_time_offset);
...
void LIVMapper::initializeComponents() 
{
  ...
  nh.param<bool>("vio/ncc_en", vio_manager->ncc_en, false);
  nh.param<double>("vio/ncc_thre", vio_manager->ncc_thre, 0.6);
  nh.param<double>("vio/huber_delta", vio_manager->huber_delta, 20.0);
  nh.param<int>("vio/min_visual_points", vio_manager->min_visual_points, 3);
  nh.param<double>("vio/min_inv_exposure", vio_manager->min_inv_exposure, 0.05);
  nh.param<double>("vio/max_inv_exposure", vio_manager->max_inv_exposure, 20.0);
  nh.param<double>("vio/min_patch_variance", vio_manager->min_patch_variance, 1.0);
  runtime_checks::require(std::isfinite(vio_manager->ncc_thre) && vio_manager->ncc_thre >= -1.0 && vio_manager->ncc_thre <= 1.0,
                          "vio/ncc_thre must be in [-1, 1]");
  runtime_checks::positive(vio_manager->huber_delta, "vio/huber_delta");
  runtime_checks::positive(vio_manager->min_inv_exposure, "vio/min_inv_exposure");
  runtime_checks::positive(vio_manager->max_inv_exposure, "vio/max_inv_exposure");
  runtime_checks::require(vio_manager->min_inv_exposure < vio_manager->max_inv_exposure &&
                          vio_manager->min_visual_points >= 3 &&
                          std::isfinite(vio_manager->min_patch_variance) && vio_manager->min_patch_variance >= 0.0,
                          "Invalid visual exposure bounds, point count or patch variance");
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  nh.param<bool>("gps/gps_en", gps_enabled, false);
  if (gps_enabled) subGPS_pvt = nh.subscribe<gnss_comm::GnssPVTSolnMsg>(gps_topic, 2000, &LIVMapper::rtk_cbk, this);
...
bool LIVMapper::processImu() 
{
  ...
  if (!p_imu->Process2(LidarMeasures, _state, feats_undistort, T_G_to_W)) return false;
  state_time_ = LidarMeasures.lio_vio_flg == VIO ? LidarMeasures.measures.back().vio_time : LidarMeasures.measures.back().lio_time;
...
void LIVMapper::handleVIO() 
{
  ...
  if (!vio_manager->processFrame(LidarMeasures.measures.back().img, _pv_list, voxelmap_manager->voxel_map_,
                                 LidarMeasures.last_lio_update_time - _first_lidar_time)) return;
...
void LIVMapper::imu_prop_callback(const ros::TimerEvent &e)
{
  ...
  if (imu_prop_enable && !prop_imu_buffer.empty())
  {
    ...
    imu_prop_odom.child_frame_id = "aft_mapped";
    vel_i = imu_propagate.rot_end.transpose() * vel_i;
...
{
    if (!gpsMsg->valid_fix || (gpsMsg->fix_type != 3 && gpsMsg->fix_type != 4) ||
        !rtk_utils::validFix(gpsMsg->latitude, gpsMsg->longitude, gpsMsg->altitude,
                             gpsMsg->time.week, gpsMsg->time.tow, gpsMsg->h_acc, gpsMsg->v_acc)) return;
    ...
    // No online consumer exists in batch mode; do not retain an unbounded duplicate queue.
    if (rtk_en) rtk_buffer.push_back(rtk_data);
    ...
    if(trans_local_.norm() > 50.0 && rtk_ini == false && rtk_en == true)
    {
      ...
      rtk_ini = InitializeRTK();
      if (rtk_ini) ROS_INFO("RTK to LIVO TRANSFORMATION INITIALIZED");
    }
    ...
    gps_odom.header.frame_id = "map"; // GNSS ENU origin; not the local odometry origin.
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr &msg_in)
{
  ...
  cv::Mat img_cur;
  try { img_cur = getImageFromMsg(msg); }
  catch (const cv_bridge::Exception& error) {
    ROS_WARN("[VIO] Image conversion failed: %s", error.what()); return;
  }
...
void LIVMapper::publish_img_rgb(const image_transport::Publisher &pubImage, VIOManagerPtr vio_manager)
{
  ...
  out_msg.header.stamp = ros::Time().fromSec(state_time_);
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  laserCloudmsg.header.stamp = ros::Time().fromSec(state_time_); //.fromSec(last_timestamp_lidar);
  ...
  if(img_en)
  {
    ...
    laserCloudbodymsg.header.stamp = ros::Time().fromSec(state_time_);
    laserCloudbodymsg.header.frame_id = "aft_mapped";
  ...
  if (rtk_en && !rtk_ini) livo_state_buffer.push_back(livo_xyz);


  nav_msgs::Odometry odom_msg_ekf;
  odom_msg_ekf.header.stamp = ros::Time().fromSec(state_time_);
  odom_msg_ekf.header.frame_id = "camera_init";
  odom_msg_ekf.child_frame_id = "aft_mapped";
  ...
  const V3D body_velocity = _state.rot_end.transpose() * _state.vel_end;
  odom_msg_ekf.twist.twist.linear.x = body_velocity(0);
  odom_msg_ekf.twist.twist.linear.y = body_velocity(1);
  odom_msg_ekf.twist.twist.linear.z = body_velocity(2);
...
void LIVMapper::publish_visual_sub_map(const ros::Publisher &pubSubVisualMap)
{
  ...
  *sub_pcl_visual_map_pub = *laserCloudFullRes;
  if (1)
  {
    ...
    laserCloudmsg.header.stamp = ros::Time().fromSec(state_time_);
    laserCloudmsg.header.frame_id = "camera_init";
...
void LIVMapper::publish_effect_world(const ros::Publisher &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list)
{
  ...
  laserCloudFullRes3.header.stamp = ros::Time().fromSec(state_time_);
...
void LIVMapper::publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
  ...
  odomAftMapped.header.stamp = ros::Time().fromSec(state_time_); //.ros::Time()fromSec(last_timestamp_lidar);
...
{
  msg_body_pose.header.stamp = ros::Time().fromSec(state_time_);
