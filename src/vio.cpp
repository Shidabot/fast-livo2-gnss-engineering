...
{
  if (patch_size < 2 || patch_size > 32 || patch_size % 2 != 0 ||
      patch_pyrimid_level < 1 || patch_pyrimid_level > 6 || max_iterations < 1 ||
      !std::isfinite(img_point_cov) || img_point_cov <= 0.0 ||
      !std::isfinite(outlier_threshold) || outlier_threshold <= 0.0 ||
      !std::isfinite(huber_delta) || huber_delta <= 0.0 || min_visual_points < 3 ||
      !std::isfinite(min_inv_exposure) || !std::isfinite(max_inv_exposure) ||
      min_inv_exposure <= 0.0 || max_inv_exposure <= min_inv_exposure ||
      !std::isfinite(min_patch_variance) || min_patch_variance < 0.0 ||
      !std::isfinite(ncc_thre) || ncc_thre < -1.0 || ncc_thre > 1.0) {
    throw std::invalid_argument("Invalid visual patch, noise or exposure configuration");
  }
  ...
  if (width <= 0 || height <= 0 || grid_size <= 0 ||
      (grid_size <= 10 && (grid_n_height <= 0 || grid_n_height > height))) {
    throw std::invalid_argument("Invalid visual grid or camera dimensions");
  }
  if (grid_size > 10)
  {
    grid_n_width = static_cast<int>(std::ceil(static_cast<double>(width) / grid_size));
    grid_n_height = static_cast<int>(std::ceil(static_cast<double>(height) / grid_size));
  }
  else
  {
...
{
  if (!patch_tmp || level < 0 || level >= patch_pyrimid_level) return;
  if (img.empty() || img.type() != CV_8UC1 ||
      !visual_utils::patchInBounds(pc[0], pc[1], img.cols, img.rows, patch_size_half, level)) {
    std::fill(patch_tmp + patch_size_total * level, patch_tmp + patch_size_total * (level + 1), 0.0f);
    return;
  }
  const int stride = static_cast<int>(img.step);
  ...
  {
    uint8_t *img_ptr = (uint8_t *)img.data + (v_ref_i - patch_size_half * scale + x * scale) * stride + (u_ref_i - patch_size_half * scale);
    for (int y = 0; y < patch_size; y++, img_ptr += scale)
    {
      patch_tmp[patch_size_total * level + x * patch_size + y] =
          w_ref_tl * img_ptr[0] + w_ref_tr * img_ptr[scale] + w_ref_bl * img_ptr[scale * stride] + w_ref_br * img_ptr[scale * stride + scale];
    }
  }
}
                               ...
                               const int pyramid_level, const int halfpatch_size, float *patch)
{
  ...
  if (!A_cur_ref.allFinite() || std::abs(A_cur_ref.determinant()) < 1e-8) {
    std::fill(patch + patch_size_total * pyramid_level,
              patch + patch_size_total * (pyramid_level + 1), 0.0f);
    return;
  }
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  for (int i = 0; i < pg.size(); i++)
  {
    ...
    if (pt_c[2] > 0)
    {
      ...
      if (px.allFinite() && new_frame_->cam_->isInFrame(px.cast<int>(), border))
      {
        ...
        int col = int(px[0]);
        int row = int(px[1]);
        it[width * row + col] = visual_utils::nearestDepth(it[width * row + col], depth);
      }
    }
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      if (!ref_ftr || !ref_ftr->patch_ || ref_ftr->img_.empty() ||
          ref_ftr->img_.type() != CV_8UC1) continue;
      if (normal_en)
      {
        ...
        if (!A_cur_ref_zero.allFinite() || std::abs(A_cur_ref_zero.determinant()) < 1e-8) continue;
      ...
      if (!pc.allFinite() || !visual_utils::patchInBounds(pc[0], pc[1], img.cols, img.rows,
                                                        patch_size_half, 0)) continue;
      if (!A_cur_ref_zero.allFinite() || std::abs(A_cur_ref_zero.determinant()) < 1e-8 ||
          !std::isfinite(ref_ftr->inv_expo_time_) || ref_ftr->inv_expo_time_ <= 0.0) continue;
      getImagePatch(img, pc, patch_buffer.data(), 0);
      double patch_mean = 0.0, patch_variance = 0.0;
      for (int k = 0; k < patch_size_total; ++k) patch_mean += patch_wrap[k];
      patch_mean /= patch_size_total;
      for (int k = 0; k < patch_size_total; ++k) {
        const double d = patch_wrap[k] - patch_mean;
        patch_variance += d*d;
      }
      if (patch_variance / patch_size_total < min_patch_variance) continue;
      ...
      if (!std::isfinite(error) || error > outlier_threshold * patch_size_total) continue;
...
{
  G.setZero();
  if (total_points < min_visual_points) return false;
  const StatesGroup before = *state;
  compute_jacobian_time = update_ekf_time = 0.0;
  for (int level = patch_pyrimid_level - 1; level >= 0; --level)
  {
    if (inverse_composition_en) has_ref_patch_cache = false;
    const bool success = inverse_composition_en ? updateStateInverse(img, level) : updateState(img, level);
    if (!success) { *state = before; G.setZero(); updateFrameState(*state); return false; }
  }
  Matrix<double, DIM_STATE, DIM_STATE> posterior = state->cov - G * state->cov;
  posterior = (0.5 * (posterior + posterior.transpose())).eval();
  Eigen::LDLT<Matrix<double, DIM_STATE, DIM_STATE>> check(posterior);
  if (!posterior.allFinite() || check.info() != Eigen::Success || !check.isPositive()) {
    *state = before; G.setZero(); updateFrameState(*state); return false;
  }
  state->cov = posterior;
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  for (int i = 0; i < pg.size(); i++)
  {
    ...
    if (pc.allFinite() && new_frame_->cam_->isInFrame(pc.cast<int>(), border)) // 20px is the patch size in the matcher
    {
      int index = static_cast<int>(pc[1] / grid_size) * grid_n_width + static_cast<int>(pc[0] / grid_size);
  ...
  for (int i = 0; i < visual_submap->voxel_points.size(); i++)
  {
    ...
    for (auto it = pt->obs_.begin(), ite = pt->obs_.end(); it != ite; ++it)
    {
      ...
      for (auto itm = pt->obs_.begin(), itme = pt->obs_.end(); itm != itme; ++itm)
      {
        ...
        NCC_up = NCC_down1 = NCC_down2 = 0.0f;
        ...
        const double denominator = std::sqrt(NCC_down1 * NCC_down2);
        if (!std::isfinite(denominator) || denominator <= 1e-10) continue;
        NCC += fabs(NCC_up / denominator);
        count++;
      }
...
void VIOManager::precomputeReferencePatches(int level)
{
  ...
  H_sub_inv.setZero();
  inverse_reference_valid.assign(total_points, false);
  M3D p_w_hat;
  ...
  for (int i = 0; i < total_points; i++)
  {
    ...
    VisualPoint *pt = visual_submap->voxel_points[i];
    if (!pt || !pt->ref_patch) continue;
    const cv::Mat& img = pt->ref_patch->img_;
    ...
    if (!visual_utils::patchInBounds(pc[0], pc[1], img.cols, img.rows, patch_size_half, level)) continue;
    inverse_reference_valid[i] = true;
    ...
    for (int x = 0; x < patch_size; x++)
    {
      uint8_t *img_ptr = (uint8_t *)img.data + (v_ref_i + x * scale - patch_size_half * scale) * stride + u_ref_i - patch_size_half * scale;
      for (int y = 0; y < patch_size; ++y, img_ptr += scale)
      {
        float du =
            0.5f *
            ((w_ref_tl * img_ptr[scale] + w_ref_tr * img_ptr[scale * 2] + w_ref_bl * img_ptr[scale * stride + scale] +
              w_ref_br * img_ptr[scale * stride + scale * 2]) -
             (w_ref_tl * img_ptr[-scale] + w_ref_tr * img_ptr[0] + w_ref_bl * img_ptr[scale * stride - scale] + w_ref_br * img_ptr[scale * stride]));
        float dv =
            0.5f *
            ((w_ref_tl * img_ptr[scale * stride] + w_ref_tr * img_ptr[scale + scale * stride] + w_ref_bl * img_ptr[stride * scale * 2] +
              w_ref_br * img_ptr[stride * scale * 2 + scale]) -
             (w_ref_tl * img_ptr[-scale * stride] + w_ref_tr * img_ptr[-scale * stride + scale] + w_ref_bl * img_ptr[0] + w_ref_br * img_ptr[scale]));
...
{
  // The cached six-column Jacobian has no exposure derivative and assumes search level zero.
  // Use the forward formulation when either assumption does not hold.
  if (exposure_estimate_en || std::any_of(visual_submap->search_levels.begin(),
      visual_submap->search_levels.end(), [](int value) { return value != 0; })) return updateState(img, level);
  if (total_points < min_visual_points) return false;
  const StatesGroup entry = *state;
  const auto validState = [&](const StatesGroup& value) {
    return value.rot_end.allFinite() && value.pos_end.allFinite() && value.vel_end.allFinite() &&
           value.bias_g.allFinite() && value.bias_a.allFinite() && value.gravity.allFinite() &&
           std::isfinite(value.inv_expo_time) && value.inv_expo_time >= min_inv_exposure &&
           value.inv_expo_time <= max_inv_exposure;
  };
  if (!validState(entry)) return false;
  bool solved = false;
  bool validate_only = false;
  std::vector<int> previous_active;
  G.setZero();
  const int stride = static_cast<int>(img.step);
  ...
  {
    double t1 = omp_get_wtime();
    z.setZero(); H_sub.setZero();
    std::vector<int> active(total_points, 0);
    ...
    for (int i = 0; i < total_points; i++)
    {
      ...
      if (pt == nullptr || !inverse_reference_valid[i]) continue;

      V3D pf = Rcw * pt->pos_ + Pcw;
      if (!pf.allFinite() || pf[2] <= 1e-6) continue;
      pc = cam->world2cam(pf);
      if (!visual_utils::patchInBounds(pc[0], pc[1], img.cols, img.rows, patch_size_half,
                                      level)) continue;
      if (visual_submap->warp_patch[i].size() < static_cast<size_t>(warp_len)) continue;
      active[i] = 1;
      ...
      for (int x = 0; x < patch_size; x++)
      {
        ...
        for (int y = 0; y < patch_size; ++y, img_ptr += scale)
        {
          double res = w_ref_tl * img_ptr[0] + w_ref_tr * img_ptr[scale] + w_ref_bl * img_ptr[scale * stride] +
                       w_ref_br * img_ptr[scale * stride + scale] - P[patch_size_total * level + x * patch_size + y];
          z(i * patch_size_total + x * patch_size + y) = res;
          patch_error += visual_utils::huberCost(res, huber_delta);
          ...
          const double sqrt_weight = std::sqrt(visual_utils::huberWeight(res, huber_delta));
          z(row) *= sqrt_weight;
          H_sub.row(row) *= sqrt_weight;
        }
      }
    ...
    if (n_meas < min_visual_points * patch_size_total || !std::isfinite(error) ||
        !z.allFinite() || !H_sub.allFinite() ||
        (!previous_active.empty() && active != previous_active)) {
      *state = entry; G.setZero(); return false;
    }
    ...
    if (error <= last_error * (1.0f + 1e-5f) + 1e-6f)
    {
      if (iteration == max_iterations || validate_only) break; // Validate before committing.
      old_state = (*state);
      last_error = error;
      ...
      using StateMatrix = Matrix<double, DIM_STATE, DIM_STATE>;
      Eigen::LDLT<StateMatrix> prior_solver(state->cov / img_point_cov);
      if (prior_solver.info() != Eigen::Success || !prior_solver.isPositive()) {
        *state = entry; G.setZero(); return false;
      }
      const StateMatrix information = H_T_H + prior_solver.solve(StateMatrix::Identity());
      Eigen::LDLT<StateMatrix> solver(information);
      if (solver.info() != Eigen::Success || !solver.isPositive()) {
        *state = entry; G.setZero(); return false;
      }
      const StateMatrix K_1 = solver.solve(StateMatrix::Identity());
      if (!K_1.allFinite()) { *state = entry; G.setZero(); return false; }
      auto &&HTz = H_sub_T * z;
      auto vec = (*state_propagat) - (*state);
      G.block<DIM_STATE, 6>(0, 0) = K_1.block<DIM_STATE, 6>(0, 0) * H_T_H.block<6, 6>(0, 0);
      auto solution = -K_1.block<DIM_STATE, 6>(0, 0) * HTz + vec - G.block<DIM_STATE, 6>(0, 0) * vec.block<6, 1>(0, 0);
      if (!solution.allFinite()) { *state = entry; G.setZero(); return false; }
      (*state) += solution;
      if (!validState(*state)) { *state = entry; G.setZero(); return false; }
      solved = true;
...
bool VIOManager::updateState(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration <= max_iterations; iteration++)
  {
    ...
    for (int i = 0; i < total_points; i++)
    {
      ...
      if (pyramid_level < 0 || pyramid_level > 12) continue;
    ...
    if (error <= last_error * (1.0f + 1e-5f) + 1e-6f)
    {
      ...
      if ((rot_add.norm() * 57.3f < 0.001f) && (t_add.norm() * 100.0f < 0.001f) &&
          (!exposure_estimate_en || expo_add.norm() < 0.001)) EKF_end = true;
    }
    else
    {
      *state = entry; G.setZero(); return false;
...
bool VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  if (img.empty() || img.depth() != CV_8U ||
      (img.channels() != 1 && img.channels() != 3) || width <= 0 || height <= 0) {
    ROS_WARN("[VIO] Invalid image; skipping frame."); return false;
  }
  if (!state || !state_propagat || !state->rot_end.allFinite() || !state->pos_end.allFinite() ||
      !std::isfinite(state->inv_expo_time) || state->inv_expo_time < min_inv_exposure ||
      state->inv_expo_time > max_inv_exposure) {
    ROS_WARN("[VIO] Invalid input state; skipping frame."); return false;
  }
  if (width != img.cols || height != img.rows) {
    if (std::abs(img.cols * image_resize_factor - width) > 1.0 ||
        std::abs(img.rows * image_resize_factor - height) > 1.0) {
      ROS_WARN("[VIO] Image resolution does not match calibrated camera; skipping frame."); return false;
    }
  ...
  const bool updated = computeJacobianAndUpdateEKF(img);
  if (total_points >= min_visual_points && !updated) {
    ROS_WARN("[VIO] Visual update rejected; keeping input state and skipping map observation updates."); return false;
  }
