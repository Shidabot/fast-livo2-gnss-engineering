...
void loadVoxelConfig(ros::NodeHandle &nh, VoxelMapConfig &voxel_config)
{
  ...
  runtime_checks::require(voxel_config.max_layer_ >= 0 && voxel_config.max_layer_ <= 16 &&
                          voxel_config.layer_init_num_.size() > static_cast<size_t>(voxel_config.max_layer_),
                          "lio/layer_init_num must cover every layer through lio/max_layer (0..16)");
  for (int count : voxel_config.layer_init_num_)
    runtime_checks::require(count >= 3, "lio/layer_init_num entries must be >= 3");
  runtime_checks::require(voxel_config.max_points_num_ >= 3 && voxel_config.max_iterations_ > 0 &&
                          voxel_config.half_map_size > 0, "Invalid map capacity or iteration count");
  runtime_checks::positive(voxel_config.max_voxel_size_, "lio/voxel_size");
  runtime_checks::positive(voxel_config.planner_threshold_, "lio/min_eigen_value");
  runtime_checks::positive(voxel_config.sigma_num_, "lio/sigma_num");
  runtime_checks::positive(voxel_config.beam_err_, "lio/beam_err");
  runtime_checks::positive(voxel_config.dept_err_, "lio/dept_err");
  runtime_checks::positive(voxel_config.sliding_thresh, "local_map/sliding_thresh");
}
