...
public:
  ...
  double img_point_cov, outlier_threshold, ncc_thre = 0.6;
  double huber_delta = 20.0;
  int min_visual_points = 3;
  double min_inv_exposure = 0.05, max_inv_exposure = 20.0;
  double min_patch_variance = 1.0;
  ...
  std::vector<bool> inverse_reference_valid;
  ...
  bool processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time);
