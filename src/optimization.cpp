...
optimization::optimization(ros::NodeHandle &nh)
{
    ...
    nh.param<vector<double>>("gps/extrinsic_T", gps_extrinT, vector<double>{0.0, 0.0, 0.0});

    nh.param<bool>("opt/load_data", debug_mode, false);
    if (nh.hasParam("opt/debug_mode")) {
        bool legacy = false;
        nh.getParam("opt/debug_mode", legacy);
        if (nh.hasParam("opt/load_data") && legacy != debug_mode)
            throw std::invalid_argument("Conflicting opt/load_data and deprecated opt/debug_mode");
        ROS_WARN("opt/debug_mode is deprecated; use opt/load_data for replay");
        debug_mode = legacy;
    }
    ...
    nh.param<double>("opt/max_rtk_gap", max_rtk_gap_, 2.0);
    if (gps_extrinT.size() != 3 ||
        !std::all_of(gps_extrinT.begin(), gps_extrinT.end(), [](double v) { return std::isfinite(v); }) ||
        !std::isfinite(gps_offset) || !std::isfinite(max_rtk_gap_) || max_rtk_gap_ <= 0.0 ||
        !std::isfinite(rtk_cov) || rtk_cov <= 0.0 ||
        !std::isfinite(livo2_RPY_cov) || livo2_RPY_cov <= 0.0 ||
        !std::isfinite(livo2_XYZ_cov) || livo2_XYZ_cov <= 0.0) {
        throw std::invalid_argument("Invalid RTK extrinsics, time offset or noise configuration");
    }
    pubGpsOdom = nh.advertise<nav_msgs::Odometry>("/gps/backend_odometry", 1000);
    // subGPS = nh.subscribe<sensor_msgs::NavSatFix>(gps_topic, 2000, &optimization::gpsHandler, this);
    if (gps_en && !debug_mode) subGPS_pvt = nh.subscribe<gnss_comm::GnssPVTSolnMsg>(gps_topic, 2000, &optimization::gpsHandler, this);
...
void optimization::offlineOptimizationTask() {
    ...
    if (!gps_en) { ROS_INFO("[Offline Optimization] GNSS disabled."); return; }
    waitForKeyFrameIdle(3.0);
    if (!ros::ok() || optimization_shutdown_requested_.load()) return;
    {
    ...
    try {
    if (!initialAlign()) {
        ROS_ERROR("[Offline Optimization] RTK alignment failed; no graph or map will be produced.");
        return;
    }
    std::cout << "[Offline Optimization] Initial alignment done." << std::endl;
    for (auto& frame : keyFrames) frame.pose = frame.pose.compose(T_imu_rtk.inverse());
    ...
    for (int i = 0; i < numPoses; ++i) {
        keyFrames[i].pose = initialEstimate.at<gtsam::Pose3>(i).compose(T_imu_rtk.inverse());
    }
    ...
    std::cout << "[Offline Optimization] Finished." << std::endl;
        ...
        ROS_ERROR("[Offline Optimization] failed: %s", error.what());
    }
}
...
void optimization::waitForKeyFrameIdle(double idle_seconds)
{
    ...
    size_t last_count = 0;
    while (ros::ok() && !optimization_shutdown_requested_.load()) {
...
{
    std::lock_guard<std::mutex> lock(mutex);
    if (!accepting_keyframes_ || !std::isfinite(time) || !pose.matrix().allFinite() ||
        !velocity.allFinite() ||
        (!keyFrames.empty() && time <= keyFrames.back().time)) {
        return;
    }
...
bool optimization::initialAlign()
{
    ...
    gpsQueue_B.clear();
    gps_supported_.assign(keyFrames.size(), false);
    std::sort(gpsQueue.begin(), gpsQueue.end(), [](const nav_msgs::Odometry& a, const nav_msgs::Odometry& b) {
        return a.header.stamp < b.header.stamp;
    });
    gpsQueue.erase(std::unique(gpsQueue.begin(), gpsQueue.end(), [](const nav_msgs::Odometry& a, const nav_msgs::Odometry& b) {
        return a.header.stamp == b.header.stamp;
    }), gpsQueue.end());
    ...
    // Short overlap and different recording durations do not imply different epochs.
    const bool epoch_mismatch = raw_overlap <= 0.0 &&
    ...
    for (size_t i = 0; i < keyFrames.size(); ++i)
    {
        ...
        auto upper = std::lower_bound(T.begin(), T.end(), tk);
        if (upper != T.end() && *upper == tk) {
            gps_supported_[i] = true;
        } else if (upper != T.begin() && upper != T.end()) {
            gps_supported_[i] = rtk_utils::supportedTime(tk, *std::prev(upper), *upper, max_rtk_gap_);
        }
    ...
    const double t_start = std::max(keyFrames.front().time, T.front());
    ...
    // Three points are insufficient if the trajectory is stationary or collinear.
    auto observable = [](const std::vector<Eigen::Vector3d>& points) {
        Eigen::Vector3d center = Eigen::Vector3d::Zero();
        for (const auto& p : points) center += p;
        center /= points.size();
        Eigen::Matrix3d scatter = Eigen::Matrix3d::Zero();
        for (const auto& p : points) scatter += (p-center) * (p-center).transpose();
        Eigen::JacobiSVD<Eigen::Matrix3d> svd(scatter);
        return svd.singularValues()(0) > 1e-4 &&
               svd.singularValues()(1) > 1e-6 * svd.singularValues()(0);
    };
    if (!observable(A_slam_gps) || !observable(B_enu_gps)) {
        ROS_ERROR("[initialAlign] trajectory is stationary or collinear; alignment is unobservable.");
        return false;
    }
...
{
    if (initialEstimate.size() != keyFrames.size() || gpsQueue_B.size() != keyFrames.size() ||
        gps_supported_.size() != keyFrames.size()) {
        throw std::runtime_error("RTK/keyframe alignment sizes do not match");
    }
    gtSAMgraph.resize(0); 
    ...
    for (size_t i = 0; i < initialEstimate.size(); ++i)
    {
        ...
        auto gps_noise = gtsam::noiseModel::Diagonal::Variances(
            (gtsam::Vector(3) << std::max(rtk_cov, cov_x), std::max(rtk_cov, cov_y),
             std::max(1.0, cov_z)).finished());
...
{
    std::lock_guard<std::mutex> lock(mutex);
    if (!gps_en || debug_mode || !accepting_keyframes_ || !gpsMsg->valid_fix ||
        (gpsMsg->fix_type != 3 && gpsMsg->fix_type != 4) ||
        !std::isfinite(gpsMsg->vel_e) || !std::isfinite(gpsMsg->vel_n) || !std::isfinite(gpsMsg->vel_d) ||
    ...
    if (!std::isfinite(timestamp_sec) || timestamp_sec - gps_offset < 0.0 ||
        timestamp_sec - gps_offset > std::numeric_limits<uint32_t>::max()) return;
    ros::Time stamp;
    ...
    gps_odom.pose.covariance[0]  = rtk_utils::variance(gpsMsg->h_acc, rtk_cov); 
    gps_odom.pose.covariance[7]  = rtk_utils::variance(gpsMsg->h_acc, rtk_cov); 
    gps_odom.pose.covariance[14] = rtk_utils::variance(gpsMsg->v_acc, 1.0); 

    if (!gpsQueue.empty() && gps_odom.header.stamp <= gpsQueue.back().header.stamp) return;
...
void optimization::loadData(const std::string& data_dir)
{
    ...
    if (rtk_file.is_open()) {
        ...
        while (std::getline(rtk_file, line)) {
            ...
            if (!(ss >> t >> x >> y >> z) || !std::isfinite(t) || t - gps_offset < 0.0 ||
            ...
            gps_msg.pose.covariance[0] = rtk_utils::variance(h_acc, rtk_cov);
            gps_msg.pose.covariance[7] = rtk_utils::variance(h_acc, rtk_cov);
            gps_msg.pose.covariance[14] = rtk_utils::variance(v_acc, 1.0);
    ...
    while (std::getline(odom_file, line_odom) && std::getline(cov_file, line_cov)) {
        ...
        double t_odom, t_cov;
        if (!(ss_odom >> t_odom) || !(ss_cov >> t_cov) ||
            !std::isfinite(t_odom) || !std::isfinite(t_cov) || t_odom < 0.0) continue;
        ...
        double vx = 0.0, vy = 0.0, vz = 0.0;
        if (!(ss_odom >> tx >> ty >> tz >> qx >> qy >> qz >> qw)) continue;
        const double q_norm = std::sqrt(qx*qx + qy*qy + qz*qz + qw*qw);
        if (!std::isfinite(q_norm) || q_norm < 1e-12) continue;
